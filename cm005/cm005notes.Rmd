...
as.numeric("5") ##changes the character to the numeric value ##
```
...
__Exercise__: Describe this output:
  ...
  ```{r}
typeof(typeof(15.2)) ##output of typeof function is a character so typeof of a character is a character##
```
...
x<-iris$Petal.Width
hist(x)
table(x) ##tells how many of each we see ##
...
filter(gapminder, country%in% c("Canada", "Algeria")) ##alternate way of doing this##
```
...
filter(gapminder, country%in% c("Canada", "Algeria"), year<1970, year>1960) ##alternate way of doing it##
```
...
```{r}
summarize(gapminder, mean_pop=mean(pop), sd_pop=sd(pop)) ##reduced entire data frame to one row ##
```
