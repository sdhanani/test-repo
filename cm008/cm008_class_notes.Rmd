...
lapply(fit1_small, identity) ##identity just returns everything that you give it ## 
```
...
```
## Poisson regression:
glm(y ~ x1 + x2 + ... + xp, family=poisson, data=your_data_frame) ## only difference between lm and glm is that you have to include family in glm##
...
gapminder %>%
    group_by(continent) %>% 
    summarize(mean_gdpPercap = mean(gdpPercap), ##summarize collapses the data down to one row##
...
```{r}
gapminder %>% 
    group_by(country) %>%   ## use arrange function to arrange by year first ##
    mutate(pop_growth = pop - pop[1]) ##groupby function tells what to reference##
```
...
gapminder %>% 
    group_by(country) %>% 
    mutate(pop_growth = pop - pop[year==1972]) ##lag function will put everything behind by one##
```
...
## theme changes the look of the plot but not the plot itself ##
```
...
p1 + labs(x="GDP per capita",  ## doesn't work on all scales ##
          y="Life Expectancy",
          title="My Plot")
...
- `labels`. Text to replace the data value labels. Most useful for discrete data. Replaces numbers with something manual.

```{r}
## Not a good idea:
p2 + scale_colour_continuous("My odd\npopulation\nscale",
                             breaks=c(0.2, 0.7, 1.2),
                             labels=c("small", "bigger", "big")) ## don't typically want to change labels on continuous scale, more useful for discrete scales##
...
p2 + theme(legend.position = "bottom") ##this will move the legend position ##
```
...
gapminder%>%
  group_by(country)%>%
  summarise(pop_change=pop[year=1972]-pop[year=1962])
...
## this is how I did it in class before he talked about the "right" way to do it ##
gapminder%>%
  arrange(year)%>%
  group_by(country)%>%
  mutate(lag_gdp= gdpPercap-lag(gdpPercap))
...
## part 3
## it's a window function, takes a vector and returns it of the same length but works on vector as a full 
gapminder %>% 
  group_by(country) %>% 
  arrange(year)
...
gapminder %>% 
  ...
  group_by(country) %>% 
  mutate(maxgdp = log(max(gdpPercap))) %>% 
  ggplot(aes(x=year, y=popinmillions, color=maxgdp)) +
  facet_wrap(~continent) +
  geom_point() +
...
## want to change the look of the plot, use the theme function ## 0=left, 1=right, 0.5=middle
```

Random example I don't have, check online, it might be there
