...
```{r}
p + geom_point(alpha=0.25) ##this indicates 25% transparency, way of overplotting##
```
...
```{r}
p2+geom_point(aes(size=year, colour=continent), alpha=0.25)
```
...
```{r}
p2+geom_point(aes(size=year, colour=continent), alpha=0.25)+scale_x_log10(aes(x=log10(gdpPercap)))

ggplot(gapminder,
       aes(x=log(gdpPercap),y=lifeExp))+geom_point()
```
***

This has moved to cm007_ggplot2 file
