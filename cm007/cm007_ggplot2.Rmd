...
vc1 + geom_smooth(method="lm") ## there are error bars present here (the grey shading) ##
```
...
ggplot(gapminder, aes(x= year, y=lifeExp, colour=continent))+
    geom_smooth(method="lm", se=FALSE)
## fits the regression even though the data aren't plotted.
```
...
- `facet_wrap`: 1D facetting -- we'll focus on this first. Group by one variable
...
As for other arguments of `facet_wrap` that I find to be most useful, check the documentation for `scales`  (common for each plot) and `ncol` (number of columns of plots) -- and if you're brave, `labeller`. 
...
ggplot(gapminder, aes(x=year, y=lifeExp, color=continent))+
  geom_point()+
  facet_wrap(~continent)+ ##facet wrap groups by continent##
  geom_smooth(method = "lm", se=FALSE, span=0.9) ##span controls how wiggly the curve is, smaller overfits##
```

`facet_grid` puts the panels in a grid. Each row corresponds to one grouping, and each column corresponds to another grouping. Mandatory argument specification: `facet_grid(GROUPING_VARIABLE_1(yaxis) ~ GROUPING_VARIABLE_2(xaxis))`.
...
ggplot(gapminder, aes(year, lifeExp, group=country)) +
    geom_line(alpha=0.2) ##this produces a line for every country, this is a "spaghetti plot"##
```
...
ggplot(gapminder, aes(year, pop, group=country)) +
  geom_line(aes(color=gdpPercap),alpha=0.2)
